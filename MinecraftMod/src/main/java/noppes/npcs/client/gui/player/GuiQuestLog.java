...
public class GuiQuestLog extends GuiNPCInterface implements ITopButtonListener, ICustomScrollListener, IGuiData {
   ...
   public void drawScreen(int i, int j, float f) {
      ...
      if(this.noQuests) {
         this.mc.fontRenderer.drawString(StatCollector.translateToLocal("quest.noquests"), super.guiLeft + 84, super.guiTop + 80, CustomNpcResourceListener.DefaultTextColor);
      } else {
         ...
         this.mc.fontRenderer.drawString(this.data.selectedCategory, super.guiLeft + 5, super.guiTop + 5, CustomNpcResourceListener.DefaultTextColor);
         if(this.data.hasSelectedQuest()) {
            String var8;
            if(this.questDetails) {
               this.drawProgress();
               var8 = StatCollector.translateToLocal("gui.text");
               this.mc.fontRenderer.drawString(var8, super.guiLeft + 284 - this.mc.fontRenderer.getStringWidth(var8), super.guiTop + 179, CustomNpcResourceListener.DefaultTextColor);
            } else {
               this.drawQuestText();
               var8 = StatCollector.translateToLocal("quest.objectives");
               this.mc.fontRenderer.drawString(var8, super.guiLeft + 168, super.guiTop + 179, CustomNpcResourceListener.DefaultTextColor);
            }
   ...
   private void drawProgress() {
      ...
      if(complete != null && !complete.isEmpty()) {
         this.mc.fontRenderer.drawString(StatCollector.translateToLocalFormatted("quest.completewith", new Object[]{complete}), super.guiLeft + 144, super.guiTop + 105, CustomNpcResourceListener.DefaultTextColor);
      }
      ...
      for(Iterator var3 = this.data.getQuestStatus().iterator(); var3.hasNext(); yoffset += 10) {
         ...
         this.mc.fontRenderer.drawString("- " + process, super.guiLeft + 144, yoffset, CustomNpcResourceListener.DefaultTextColor);
      }
