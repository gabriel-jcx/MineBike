         ...
         if(!json.startsWith(new String[]{"]"})) {
            ...
            if(var6.func_150303_d() == 3) {
               ...
               for(i = 0; var6.tagCount() > 0; ++i) {
                  var10[i] = ((NBTTagInt)var6.removeTag(0)).func_150287_d();
               }

               return new NBTTagIntArray(var10);
            } else if(var6.func_150303_d() != 1) {
               return var6;
            } else {
               byte[] var9 = new byte[var6.tagCount()];

               for(i = 0; var6.tagCount() > 0; ++i) {
                  var9[i] = ((NBTTagByte)var6.removeTag(0)).func_150290_f();
               }
   ...
   private static NBTJsonUtil.JsonLine ReadTag(String name, NBTBase base, List list) {
      ...
      if(base.getId() == 8) {
         ...
         if(base.getId() == 9) {
            ...
            Object key1;
            for(Iterator data1 = line3.func_150296_c().iterator(); data1.hasNext(); line1 = ReadTag(key1.toString(), line3.getTag(key1.toString()), list)) {
               key1 = data1.next();
            }
